...
import React, { useState, useEffect, useMemo } from "react";
import axios from "axios";
import { Users, Search, Filter, Loader2, ChevronLeft, ChevronRight, MoreHorizontal } from "lucide-react";
import { Link } from "react-router-dom";
...
const EmployeeTable = ({ onStatsCalculated }) => {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter(
      (employee) =>
        ...
        employee.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        employee.role?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        employee.departmentId?.name?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [employees, searchTerm]);

  const totalPages = Math.ceil(filteredEmployees.length / EMPLOYEES_PER_PAGE);
  const startIndex = (currentPage - 1) * EMPLOYEES_PER_PAGE;
  const employeesToDisplay = filteredEmployees.slice(startIndex, startIndex + EMPLOYEES_PER_PAGE);
  ...
  if (loading) return <div className="h-64 flex items-center justify-center bg-white rounded-2xl border border-slate-200 shadow-sm"><Loader2 className="animate-spin text-blue-600" /></div>;
  if (error) return <div className="h-64 flex items-center justify-center bg-rose-50 rounded-2xl border border-rose-100 text-rose-600">{error}</div>;
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-full">
      ...
      <div className="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
           ...
           <p className="text-sm text-slate-500 hidden sm:block">{employees.length} total employees</p>
        </div>
      ...
      <div className="overflow-x-auto flex-1">
        <table className="w-full">
          <thead className="bg-slate-50/50">
            <tr>
              {["Employee", "Role", "Department", "Status", ""].map((header) => (
                <th key={header} className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider first:pl-6 last:pr-6">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {employeesToDisplay.length === 0 ? (
              ...
              employeesToDisplay.map((emp) => (
                <tr key={emp._id} className="group hover:bg-blue-50/30 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-slate-900 text-sm">{emp.username}</div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full border ${getStatusClass(emp.status)}`}>
                      {emp.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <Link to={`/employees/${emp._id}`} className="text-slate-400 hover:text-blue-600 transition-colors p-2 hover:bg-blue-50 rounded-lg inline-block">
