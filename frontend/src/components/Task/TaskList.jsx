            ...
            <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col items-end min-w-[150px]">
                    ...
                    <p className='text-xs text-gray-600 mt-1'>
                        Assigned: <span className='font-medium text-gray-800'>{assigneeName}</span>
                    </p>

                    <div className="mt-3 flex space-x-2">
                        {/* Only show AddEmpToTaskBtn for the assigned Manager */}
                        {userRole === "Manager" && task.assignedManager?._id === userId && (
                            // NOTE: AddEmpToTaskBtn is a custom component, assuming it handles its own styling and props
                            <AddEmpToTaskBtn taskId={task._id} taskData={[]} className="text-xs p-1" />
                        )}
...
// --- TaskList Component ---
const TaskList = ({ tasks, loading, getNoTasksMessage, navigate, handleDeleteTask, user }) => {
    // Note: The new design had filtering logic directly in TaskList using a local `tasks` array.
    // In this refactoring, we assume the `tasks` prop is already the filtered list from the parent.
