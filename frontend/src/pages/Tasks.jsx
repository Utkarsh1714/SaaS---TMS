...
import { useNotifications } from '@/context/NotificationContext'
import Sidebar from '@/components/Layout/Sidebar'
import { SearchIcon, BellIcon, UserIcon, RefreshCcw, XIcon, BellDot } from 'lucide-react'
...
const TasksPage = () => {
    ...
    const [activeFilter, setActiveFilter] = useState("None");
    const navigate = useNavigate();
    const { addNotification, toggleNotificationPanel, notifications } = useNotifications();
    ...
    const fetchTasks = useCallback(async (filter) => {
        const currentFilter = filter || activeFilter;
        let endpoint = user.role === "Boss" ? `api/task/boss` :
                       user.role === "Manager" ? `api/task/manager` :
                       `api/task/employee`;
        let queryParams = "";
    ...
    const getNoTasksMessage = () => {
        ...
        if (activeFilter === "None") {
            ...
            return "No tasks have been created yet. Please check back later or create a new task.";
        } else if (["Completed", "In Progress", "Overdue", "Pending"].includes(activeFilter)) {
            return `No tasks found with the status '${activeFilter}'.`;
        } else if (activeFilter.includes("priority") || activeFilter.includes("Created")) {
            return `No tasks found when filtered by '${activeFilter}'.`;
        }
        return "No tasks available for this filter."; // Fallback message
    };
                    ...
                    <div className="px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between h-16 gap-4 sm:gap-0">
                            ...
                            <div className="flex items-center justify-center gap-2">
                                ...
                                {/* Notifications Button (Placeholder) */}
                                <button onClick={toggleNotificationPanel}  className="flex-shrink-0 p-1 text-gray-400 rounded-full hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                    <span className="sr-only">View notifications</span>
